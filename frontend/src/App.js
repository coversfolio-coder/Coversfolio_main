      ...
      <header className="topbar">
        ...
        <div className="nav-right">
          <div style={{ position: "relative" }}>
            <button className="icon-button" aria-label="Search" title="Search" data-testid="search-button" onClick={() => { setSearchOpen((v) => !v); setNotifOpen(false); }}><Search size={19} /></button>
            {searchOpen && (
              <>
                  ...
                  <div className="claim-card" key={claim.id} data-testid={`packet-row-${claim.id}`}>
                    ...
                    <div className="claim-bottom-row">
                      ...
                      <div style={{ display: "flex", gap: 6 }}>
                        ...
                        {canEdit && (
                          <button className="icon-button" aria-label="Remove claim" title="Remove claim" data-testid={`remove-claim-${claim.id}`} onClick={() => deleteClaim(claim)}>
                            <Trash2 size={14} />
                          </button>
                        )}
                      </div>
                        ...
                        <label style={{ fontSize: 11 }}>Description
                          <div style={{ display: "flex", gap: 6 }}>
                            ...
                            {landingStatsEdit.length > 1 && (
                              <button type="button" className="icon-button" aria-label="Remove stat" title="Remove stat" onClick={() => setLandingStatsEdit((prev) => prev.filter((_, pi) => pi !== i))} data-testid={`remove-landing-stat-${i}`}><Trash2 size={14} /></button>
                            )}
                          </div>
                        </label>
                      </div>
