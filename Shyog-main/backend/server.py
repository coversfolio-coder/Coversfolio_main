...
async def register(input: AuthInput, request: Request, response: Response):
    ...
    user = {"id": str(uuid.uuid4()), "email": email, "name": input.name, "password_hash": hash_password(input.password), "household_id": household_id, "role": "owner", "created_at": datetime.now(timezone.utc).isoformat()}
...
async def get_dashboard(user: dict = Depends(current_user)):
    ...
    for policy in policies:
        ...
        policy_summaries.append({
            "id": policy["id"], "insurer_name": policy["insurer_name"], "policy_type": policy.get("policy_type"),
    ...
    claims = [{k: v for k, v in c.items() if k not in ("household_id", "created_by", "created_at", "notes", "queries", "settlements", "stage_history", "checklist")} for c in full_claims]
